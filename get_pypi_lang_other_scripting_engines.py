...
build_name = 'lang_other_scripting_engines.dat'

# Page Iteration Count
pg_count = 0

if os.path.exists('./data/' + build_name):
    with open('./data/' + build_name, 'r') as fo:
        for line in fo:
            line = line.strip()
            pg_count = int(line)
...
def main_funk(dl_loc):
    ...
    try:
        # Set Initial URL
        url = "https://pypi.org/search/?q=&o=&c=Programming+Language+%3A%3A+Other+Scripting+Engines"
        ...
        for pg_list_2s in pg_list_2:
            ...
            for link in soup.find_all('a'):
                ...
                if href != None and '/project/' in href:
                    ...
                    # Unique Directory & Complimentary Description File (Use When Look For Suitable Archived Modules)
                    fname = pjt_n + '.txt'
                    ...
                    for link in soup.find_all('a'):
                        ...
                        if href2 != None and 'https://files.pythonhosted.org/packages/' in href2:
                            ...
                            if not os.path.exists(dl_fullpath):
                                ...
                                # Codecs Module To Write The Description File From Harvested Data Using Beautiful Soup
                                ...
                                # Download Package(s)
                                print('page:', pg_count, '/', page_max, 'saving:', pac_name_ver)
                            ...
                            # Drop In Here If Module Version Already Exists Locally
                                ...
                                print('page:', pg_count, '/', page_max, 'skipping:', pac_name_ver)

                            # Iteration Count Downloads Per Module
                            i_2 += 1

            # Update Page Count In Data File
            distutils.dir_util.mkpath('./data')
            cat_dat = './data/' + build_name
            pg_count_str = str(pg_count)
            with open(cat_dat, 'w') as fo:
                fo.write(pg_count_str)
            fo.close()
        ...
        if os.path.exists(dl_fullpath):
            os.remove(dl_fullpath)
        handler()
...
with open('config.txt', 'r') as fo:
    for line in fo:
        ...
        if line.startswith('DEST: ') and os.path.exists(line.replace('DEST: ', '')):
            ...
            dl_loc = dl_loc + "/lang_other_scripting_engines"
